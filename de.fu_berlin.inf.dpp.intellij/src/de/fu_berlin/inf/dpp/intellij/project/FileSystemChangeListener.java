...
public class FileSystemChangeListener extends AbstractStoppableListener
    ...
    private void generateFileMove(SPath oldSPath, SPath newSPath,
        ...
        // File moved activity does not include the file's contents, so we do not need to save the document before reading the file from the disk
        if (before) {
...
//            editorManager.saveFile(newSPath);
    ...
    public void contentsChanged(
        ...
        if (virtualFileEvent.isFromSave() || !virtualFileEvent.getFile().exists()) {
            return;
        }
